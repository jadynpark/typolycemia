...
expName = 'typo_VersionB'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/Jadyn/Desktop/typo/typo_VersionB_lastrun.py',
...
block3Intro = visual.TextStim(win=win, name='block3Intro',
    text='Great!\n\nYou will continue reading sentences out loud. Please do not worry about the meaning and focus on reading the sentences as fast and accurate as you can.\n\nPress the spacebar to proceed.',
    font='Open Sans',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
word_introComponents = [wordInst, proceed1]
...
_proceed1_2_allKeys = []
# keep track of which components have finished
word_intro2Components = [wordInst2, proceed1_2]
...
while continueRoutine:
    ...
    # *proceed1_2* updates
    waitOnFlip = False
    if proceed1_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        proceed1_2.frameNStart = frameN  # exact frame index
        proceed1_2.tStart = t  # local t and not account for scr refresh
        proceed1_2.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(proceed1_2, 'tStartRefresh')  # time at next scr refresh
        proceed1_2.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(proceed1_2.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(proceed1_2.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if proceed1_2.status == STARTED and not waitOnFlip:
        theseKeys = proceed1_2.getKeys(keyList=['space'], waitRelease=False)
        _proceed1_2_allKeys.extend(theseKeys)
        if len(_proceed1_2_allKeys):
            proceed1_2.keys = _proceed1_2_allKeys[-1].name  # just the last key pressed
            proceed1_2.rt = _proceed1_2_allKeys[-1].rt
            # a response ends the routine
            continueRoutine = False
...
# check responses
if proceed1_2.keys in ['', [], None]:  # No response was made
    proceed1_2.keys = None
thisExp.addData('proceed1_2.keys',proceed1_2.keys)
if proceed1_2.keys != None:  # we had a response
    thisExp.addData('proceed1_2.rt', proceed1_2.rt)
thisExp.addData('proceed1_2.started', proceed1_2.tStartRefresh)
thisExp.addData('proceed1_2.stopped', proceed1_2.tStopRefresh)
thisExp.nextEntry()
# the Routine "word_intro2" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()

# set up handler to look after randomisation of conditions etc
wordPracLoop = data.TrialHandler(nReps=1.0, method='sequential', 
...
# completed 1.0 repeats of 'wordPracLoop'
...
for thisWordLoop in wordLoop:
    ...
    while continueRoutine:
        ...
        # *proceed3* updates
        if proceed3.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
...
# set up handler to look after randomisation of conditions etc
sentPracLoop = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
...
# completed 1.0 repeats of 'sentPracLoop'
...
for thisNonsenseLoop in nonsenseLoop:
    ...
    # keep track of which components have finished
    meaninglessComponents = [nonsenseStim, nonsenseResp, proceed8, nonsenseBeep, ITI4]
    ...
    nonsenseLoop.addData('ITI4.started', ITI4.tStartRefresh)
    nonsenseLoop.addData('ITI4.stopped', ITI4.tStopRefresh)
...
for thisMeaningLoop in meaningLoop:
    ...
    # keep track of which components have finished
    meaningfulComponents = [sentStim, sentResp, proceed6, sentBeep, ITI5]
    ...
    meaningLoop.addData('ITI5.started', ITI5.tStartRefresh)
    meaningLoop.addData('ITI5.stopped', ITI5.tStopRefresh)
