...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/Jadyn/Desktop/typo/typoExperiment_lastrun.py',
...
block2Intro = visual.TextStim(win=win, name='block2Intro',
    text='You will now begin the actual task. \n\nSometimes, the sentences may not make sense. For example, you will see sentences like “The loud grape was angry to support her dream.”\n\nPlease do not worry about the meaning and focus on reading the sentences out loud.\n\nWhen you’re ready, press space bar to proceed.',
    font='Arial',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
for thisWordPracLoop in wordPracLoop:
    ...
    # keep track of which components have finished
    word_practiceComponents = [wordPrac, wordPrac_resp, beepPrac, proceedPrac]
    ...
    while continueRoutine:
        ...
        # *wordPrac* updates
        if wordPrac.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
...
while continueRoutine:
    ...
    # *proceed2* updates
    if proceed2.status == NOT_STARTED and t >= 2.0-frameTolerance:
        # keep track of start time/frame for later
...
wordLoop = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('word_cond.xlsx', selection='1:3'),
...
for thisWordLoop in wordLoop:
    ...
    # keep track of which components have finished
    wordsComponents = [wordStim, wordResp, proceed3, wordBeep]
    ...
    while continueRoutine:
        ...
        # start/stop wordBeep
        if wordBeep.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
...
sentPracLoop = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('sentprac.xlsx'),
...
for thisSentPracLoop in sentPracLoop:
    ...
    # keep track of which components have finished
    sen_practiceComponents = [sentPrac, sentPrac_resp, beepSentPrac, proceedSent]
...
blocks = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions("counter"+expInfo['group']+".xlsx"),
...
for thisBlock in blocks:
    ...
    # keep track of which components have finished
    sentencesComponents = [sentStim, sentResp, proceed8, sentBeep]
    ...
    while continueRoutine:
        ...
        # *sentStim* updates
        if sentStim.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        # start/stop sentBeep
        if sentBeep.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
    ...
    for thisComponent in sentencesComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    blocks.addData('sentStim.started', sentStim.tStartRefresh)
    blocks.addData('sentStim.stopped', sentStim.tStopRefresh)
    # check responses
    if sentResp.keys in ['', [], None]:  # No response was made
        sentResp.keys = None
    blocks.addData('sentResp.keys',sentResp.keys)
    if sentResp.keys != None:  # we had a response
        blocks.addData('sentResp.rt', sentResp.rt)
    blocks.addData('sentResp.started', sentResp.tStartRefresh)
    blocks.addData('sentResp.stopped', sentResp.tStopRefresh)
    blocks.addData('proceed8.started', proceed8.tStartRefresh)
    blocks.addData('proceed8.stopped', proceed8.tStopRefresh)
    sentBeep.stop()  # ensure sound has stopped at end of routine
    blocks.addData('sentBeep.started', sentBeep.tStartRefresh)
    blocks.addData('sentBeep.stopped', sentBeep.tStopRefresh)
