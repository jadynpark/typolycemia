...
function wordInstRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of wordInstComponents)
...
function pracInst_wordRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of pracInst_wordComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function pracLoopWordLoopBegin(pracLoopWordLoopScheduler) {
  ...
  for (const thisPracLoopWord of pracLoopWord) {
...
function sentLoopPracLoopBegin(sentLoopPracLoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisSentLoopPrac of sentLoopPrac) {
...
function senLoopLoopBegin(senLoopLoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisSenLoop of senLoop) {
...
function wordPracRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of wordPracComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function greatRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of greatComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function senInstRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of senInstComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function pracInstSentRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of pracInstSentComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function senPracRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of senPracComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function great_2RoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of great_2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function sentencesRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of sentencesComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
